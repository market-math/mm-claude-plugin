...
- **First pass:** spawn `staff-engineer` with the Agent tool. Keep the returned `agentId`.
- **Later passes:** continue that same agent with `SendMessage` to its `agentId` (its name also works),
